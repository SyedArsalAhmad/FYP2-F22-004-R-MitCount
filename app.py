...
mysql_password = '1234'
mysql_db = 'mbilal'
conn = mysql.connect(host=mysql_host, user=mysql_user, password=mysql_password, database=mysql_db)
cursor = conn.cursor()
personid = 0
@app.route('/')
def index():
...
def mylogin():
    if request.method == 'POST':
        ...
        if user:
            ...
            if user[3] == hashlib.sha256(password.encode('utf-8') + email.encode('utf-8')).hexdigest():
                print("Login hased password",hashed_password)
...
def register():
    if request.method == 'POST':
        ...
        email = request.form['email']
        password = request.form['password']
        hashed_password = hashlib.sha256(password.encode('utf-8') + email.encode('utf-8')).hexdigest()
